...
class LoginController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		$session->loginTries ++;
		$this->view->form = new ZendBasics_Form_Login($session->loginTries);
	}

	public function submitAction() {
		$session = new Zend_Session_Namespace('login');
		$form = new ZendBasics_Form_Login($session->loginTries);
		if ($this->getRequest()->isPost()) {
			if ($form->isValid($this->getRequest()->getPost())
					&& ZendBasics_Auth::isAuthenticated($form->getValues())) {
				$this->view->isAuthenticated = true;
				$session->loginTries = 0;
				//identity object should be stored in session
				$memberTable = new ZendBasics_ModelTable_Members();
				$member = $memberTable->getMemberById(ZendBasics_Auth::getInstance()->getIdentity()->id);
				$session = new Zend_Session_Namespace('ZendBasics');
	...
	public function resetAction() {
		...
		if (!$this->view->__isset('form') || !$this->getRequest()->isPost()) {
			$this->view->form = new ZendBasics_Form_Reset();
		}
	}
	...
	public function resetsubmitAction() {
		...
		if ($this->getRequest()->isPost()
				...
				if ($member = $memberTable->getMemberByEmail($this->view->form->getValue('email'))) {
					...
					$resetTable = new ZendBasics_ModelTable_MembersReset();
					$resetCode = $resetTable->saveNewReset($member);
					// Send reset code to user email
					$sendReset = new ZendBasics_Mail_SendReset();
					$sendReset->setResetCode($resetCode);
					$sendReset->setMember($member);
					$sendReset->setHost($this->getRequest()->getHttpHost());
					$sendReset->sendEmail();
	...
	public function resetmailsubmitAction() {
		...
		if ($this->getRequest()->getParam('id') && $validator->isValid($this->getRequest()->getParam('id'))) {
			...
			if ($memberExist = $resetTable->checkCodeByMemberID($memberID,$resetCode)) {
				$this->view->form = new ZendBasics_Form_Changepassword();
				# set hidden value
