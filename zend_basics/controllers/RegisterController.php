...
class RegisterController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		if (!$this->view->__isset('form') || get_class($this->view->form) != 'ZendBasics_Form_Register_NewMember') {
			...
			$session->loginTries ++;
			$this->view->form = new ZendBasics_Form_Register_NewMember($session->loginTries);
		}
	}
	...
	public function submitAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			$memberTable = new ZendBasics_ModelTable_Members();
			$error = array();
			//verify form data
	...
	public function verifyAction() {
		$session = new Zend_Session_Namespace('verify');
		$validator = new ZendBasics_Validate_Md5();
		switch (true) {
			case get_class($session->member) == 'ZendBasics_Model_Member':
		...
		$this->view->form = new ZendBasics_Form_Register_Verify();
	...
	public function verifysubmitAction() {
		...
		if ($this->view->form->isValid($request)) {
			$membersVerifyTable = new ZendBasics_ModelTable_MembersVerify();
			if ($member_verify = $membersVerifyTable->checkCodeByMemberID($member, $this->view->form->getValue('code'))) {
				...
				//$addStartCapital = new ZendBasics_ModelTable_MembersMoney();
